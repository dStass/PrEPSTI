...
public abstract class Agent {
    ...
    static public void SET_RAND(final long seed) {
        RANDOM_SEED = seed;
     ...
     * @return
     */
    static public String REINIT(final ArrayList<Agent> agentList, final int year) {
        String report = "";
    ...
    /**
     * Tests, given by per 1000 per year, from 2007-2018 Table 14 ARTB 2018
     */
     ...
     * @throws Exception
     */
    static private String REINIT_SCREEN_CYCLE(final ArrayList<Agent> agentList, int year) throws Exception {
        final String report = "";
        // double[] testRates = new double[]
        // {333,340,398,382,383,382,391,419,445,499,488,488,488} ;
        // 2007 - 2009
        // 333,340,398,
        ...
        final double ratio = testBase / TEST_RATES[year];
    ...
    // current partners
    private final ArrayList<Integer> currentPartnerIds = new ArrayList<Integer>();

    private final ArrayList<Relationship> currentRelationships = new ArrayList<Relationship>();
    ...
    /**
     * Whether any Site of the Agent is showing symptoms of the STI. Should be false
     * if infecteStatus is false.
     */
    ...
    /**
     * probability of using condom in a Casual Relationship even when apparently
     * safe (PrEP, TasP, etc)
     */
    protected double probabilityUseCondomCasual = RAND.nextDouble();

    /**
     * probability of using condom in a Regular Relationship even when apparently
     * safe (PrEP, TasP, etc)
     */
    protected double probabilityUseCondomRegular = RAND.nextDouble();

    /**
     * probability of using condom in a Monogomous Relationship even when apparently
     * safe (PrEP, TasP, etc)
     */
     ...
     */
    public static Site[] CHOOSE_SITES(final Agent agent0, final Agent agent1, final String relationshipClazzName) {
        return Agent.CHOOSE_SITES(agent0, agent1);
    }

    /**
     * TODO: Implement Method based on relative frequency of types of sexual
     * contact.
     * 
     * @param agent0
     * @param agent1
     ...
     * @return Probabilistically chosen Site[2] of Sites for sexual contact.
     */
    public static Site[] CHOOSE_SITES_NEW(final Agent agent0, final Agent agent1, final String relationshipClazzName) {
     ...
     * @return Probabilistically chosen Site[2] of Sites for sexual contact.
     */
    public static Site[] CHOOSE_SITES(final Agent agent0, final Agent agent1) {
        final Site site0 = agent0.chooseSite();
        final Site site1 = agent1.chooseSite(site0);
     ...
     */
    public static boolean USE_CONDOM(final Agent agent0, final Agent agent1, final String relationshipName) {
     ...
     * @return
     */
    static public ArrayList<Agent> REBOOT_AGENTS(final String simName) {
        final ArrayList<Agent> agents = new ArrayList<Agent>();
        final String folderPath = FOLDER_PATH;
        final String SITE = "Site:";

        // Needed if rebootFile == false
        final ArrayList<String> birthReport = new ArrayList<String>();
        final ArrayList<ArrayList<Comparable>> agentDeathReport = new ArrayList<ArrayList<Comparable>>();
        // String screeningRecord = "" ;
        {
            final String rebootFileName = simName.concat("-REBOOT.txt");
            try {
                final BufferedReader fileReader = new BufferedReader(new FileReader(folderPath + rebootFileName));
                // Find Agents line in -REBOOT.txt file . Should be the first line
                for (String record = ""; record != null; record = fileReader.readLine()) {
                    final int agentIndex = record.indexOf(Reporter.AGENTID);
                    if (agentIndex > 0) {
        ...
        String className;
        Site[] sites;
        final ArrayList<String> siteNames = new ArrayList<String>(Arrays.asList(MSM.SITE_NAMES)); // TODO: Replace with
                                                                                                  // Agent.SITE_NAMES
        String infectionString;
        // int daysPerYear = 365 ;

        // ArrayList deadAgentIds = new ArrayList<Object>() ; // Get ArrayList of dead
        // Agents so we don't waste time reading their data

        // Reboot saved Agent data
        int maxAgentId = 0;
        for (final String birthRecord : birthReport) {
            final ArrayList<String> birthList = Reporter.EXTRACT_ARRAYLIST(birthRecord, AGENTID);
            if (birthList.isEmpty())
                continue;
            final ArrayList<String> properties = Reporter
                    .IDENTIFY_PROPERTIES(birthList.get(0).substring(0, birthList.get(0).indexOf(SITE)));

            for (final String birth : birthList) {
                ...
                try {
                    ...
                    final MSM newAgent = new MSM(startAge);
                    ...
                    final String stringSite = infectionString.substring(infectionString.indexOf(SITE));
                    ...
                    for (final Site site : sites) {
                        ...
                        try {
                            for (final String property : siteProperties) {
                                ...
                                if ("symptomatic".equals(property)) {
                                    ...
                                    if (!Reporter.CLEAR.equals(valueString)) {
                                        ...
                                        final boolean oldSymptomatic = newAgent.getSymptomatic();
                                        ...
                                        // TODO: Generalise for multiple arbitrary Site properties, as with newAgent
                                        // above
                                        newAgent.setInfectionTime(
                                                Integer.valueOf(Reporter.EXTRACT_VALUE("infectionTime", stringSite)),
                                                site);
                                    }
                                    continue;
                                }
                                ...
                                final Method getterMethod = Site.class.getDeclaredMethod(getterName);
                                final Class propertyClazz = getterMethod.getReturnType();
                                ...
                                final Method valueOfMethod = valueOfClazz.getMethod("valueOf", String.class);
                                // LOGGER.log(Level.INFO,"{1} {0}", new Object[]
                                // {valueOfMethod.invoke(null,valueString),property});

                                final String setterName = "set" + property.substring(0, 1).toUpperCase()
                                        + property.substring(1);
                                final Method setMethod = Site.class.getDeclaredMethod(setterName, propertyClazz);
                                setMethod.invoke(site, valueOfMethod.invoke(null, valueString));
                            }

                        } catch (final Exception e) {
                            LOGGER.severe(e.toString());
                            // LOGGER.log(Level.SEVERE, "{0} {1} {2}", new Object[] {propertyClazz,
                            // valueString}) ;
                        }
                    }
                    // if (uninfected)
                    // LOGGER.info(infectionString) ; //
                    // (String.valueOf(newAgent.getInfectedStatus()));
                } catch (final Exception e) {
    ...
    /**
     * Restores individual Agents' properties from saved census data from previous
     * simulation.
     * 
     * @param newAgent
     * @param census
     * @param propertyArray
     */
    static void REBOOT_AGENT(final Agent newAgent, final String census, final ArrayList<String> propertyArray) {
        propertyArray.remove("agent");
        propertyArray.remove("age");

        final Field[] agentDeclaredFields = Agent.class.getDeclaredFields();

        final HashMap<Class, Field[]> clazzFields = new HashMap<Class, Field[]>();
        ...
        for (Class agentClazz = newAgent.getClass(); !Agent.class.equals(agentClazz); agentClazz = agentClazz
                .getSuperclass())
        ...
        try {
            for (final String property : propertyArray) {
                ...
                for (final Class agentClazz : clazzFields.keySet())
                    for (final Field field : clazzFields.get(agentClazz))
     ...
     */
    public Agent(final int startAge) {
    ...
    private void setAgentId(final int id) {
        this.agentId = id;
    }
     ...
     * 
     * @param startAge = -1, choose random startAge from 16 to 65, for initialising
     *                 community = 0 , choose random startAge from 16 to 25, new
     *                 agents entering sexual maturity > 0 , take startAge as given
     */
    private int initAge(final int startAge) {
        int ageYears;
     ...
     * @param startAge
     */
    final public void initInfectedStatus(final int startAge) {
        if (startAge >= 0)
            return;
     ...
     * @param ageDays
     */
    public void setAge(final int ageYears, final int ageDays) {
     ...
     */
    public void setConcurrency(final int newConcurrency) {
    ...
    /**
     * Initialises screenCycle from a Gamma distribution to determine how often an
     * Agent is screened, and then starts the cycle in a random place so that not
     * every Agent gets screened at the same time.
     * 
    ...
    protected int sampleGamma(final double shape, final double scale, final double rescale) {
     ...
     * 
     * @return riskyStatusCasual (boolean) whether the Agent practices risky
     *         behaviour regarding condom use within Casual Relationships.
     */
     ...
     * 
     * @return riskyStatusRegular (boolean) whether the Agent practices risky
     *         behaviour regarding condom use within Regular Relationships.
     */
    ...
    private void initInfidelity() {
        final int maximum = getMaxRelationships();
     ...
     */
    public void setInfidelity(final double newInfidelity) {
    ...
    public String getCensusReport() {
        ...
        censusReport += Reporter.ADD_REPORT_PROPERTY("age", getAge()); // Reporter.ADD_REPORT_PROPERTY("startAge",
                                                                       // getAge()) ;
        ...
        /*
         * Class fieldClazz ; Class agentClazz ; String getterName ; Method getMethod ;
         * for (String fieldName : getCensusFieldNames() ) { censusReport +=
         * Reporter.ADD_REPORT_PROPERTY(fieldName,Agent.class.getField(fieldName).get) ;
         * try { //fieldClazz = this.getClass().getField(fieldName).getClass() ;
         * agentClazz = Agent.class ; // Class.forName("String") ; getterName = "get" +
         * fieldName.substring(0,1).toUpperCase() + fieldName.substring(1) ; getMethod =
         * agentClazz.getMethod(getterName, (Class[]) null) ; censusReport +=
         * Reporter.ADD_REPORT_PROPERTY(fieldName,getMethod.invoke(this)) ; } catch (
         * NoSuchMethodException nsme) { LOGGER.log(Level.INFO, "NSME {0} {1}", new
         * Object[]{fieldName, nsme.getLocalizedMessage()}); } catch
         * (IllegalAccessException iae) {
         * Logger.getLogger(Agent.class.getName()).log(Level.SEVERE, null, iae); } catch
         * (IllegalArgumentException iae) {
         * Logger.getLogger(Agent.class.getName()).log(Level.SEVERE, null, iae); } catch
         * (InvocationTargetException ite) {
         * Logger.getLogger(Agent.class.getName()).log(Level.SEVERE, null, ite); }
         * //catch ( ClassNotFoundException cnfe) { // LOGGER.log(Level.INFO,
         * "CNFE {0} {1}", new Object[]{fieldName, cnfe.getLocalizedMessage()}); }
         * //catch (NoSuchFieldException nsfe) { // LOGGER.log(Level.INFO, "{0} {1}",
         * new Object[]{fieldName, nsfe.getMessage()}) ; } }
         */
        return censusReport;
    }
    ...
    /**
     * 
     * @return (String) List of property values needed to pick up the simulation
     *         where it left off.
     */
    public String getRebootData() {
        final String report = getCensusReport();
        // Infection status
        /*
         * if (infectedStatus) for (Site site : getSites()) if (site.getInfectedStatus()
         * > 0) { report +=
         * Reporter.ADD_REPORT_PROPERTY(site.toString(),site.getSymptomatic()) ; report
         * += Reporter.ADD_REPORT_PROPERTY("infectionTime",site.getInfectionTime()) ; }
         */
        return report;
    }
     ...
     * @return randomly chosen available Site
     */
    protected Site chooseSite(final Site site) {
        return chooseSite();
    }
     ...
     * Called to adjust condom use to reflect behavioural trends.
     * 
     * @param parameter (double) Factor between zero and one for scaling down the
     *                  probability of using a condom.
     */
    public void adjustCondomUse(final double parameter) {
        adjustProbabilityUseCondom(parameter);
    }

    /**
     * Randomly scales down probabilityUseCondom to no less than bottom times its
     * current value.
     * 
     * @param bottom - probabilityUseCondom scales no lower than this.
     */
    public void adjustProbabilityUseCondom(final double bottom) {
     ...
     */
    public void setProbabilityUseCondom(final double useCondom) {
     ...
     * 
     * @param useCondomMonogomous(double) New probability of using a condom in a
     *                                    Regular Relationship.
     */
    public void setProbabilityUseCondomMonogomous(final double useCondomMonogomous) {
     ...
     * Setter for probabilityUseCondomRegular.
     * 
     * @param useCondomRegular (double) New probability of using a condom in a
     *                         Regular Relationship.
     */
    public void setProbabilityUseCondomRegular(final double useCondomRegular) {
     ...
     *                        Relationship.
     */
    public void setProbabilityUseCondomCasual(final double useCondomCasual) {
     ...
     * 
     * @return (double) The probably of choosing to use a condom when the choice is
     *         made.
     */
     ...
     */
    protected double scaleProbabilityUseCondom(final double scale) {
    ...
    /**
     *
     * Rescales the probability of using a condom in a Casual Relationship by factor
     * scale
     * 
     * @param scale
     */
    protected double scaleProbabilityUseCondomCasual(final double scale) {
     ...
     * @param scale
     */
    protected double scaleProbabilityUseCondomRegular(final double scale) {
    ...
    /**
     * Incorporate the effects of aging on sexual activity. Concurrency and
     * infidelity decrease from startAge == 30
     * 
     * @return (String) Description of any altered quantities if any, empty
     *         otherwise
     */
     ...
     */
    public boolean receiveInfection(final double transmitProbability, final Site site) {
     ...
     */
    public boolean getSymptomatic(final Site site) {
     ...
     * @return
     */
    public boolean setSymptomatic(final boolean newSymptomatic) {
     ...
     * @return
     */
    public boolean setSymptomatic(final Site site) {
     ...
     * @return
     */
    public boolean setSymptomatic(final boolean symptoms, final Site site) {
    ...
    /**
     * Calls site.infectionTime() to set the number of days left for the untreated
     * infection to clear. Used for rebooting Agents from previous simulation.
     * 
     * @param infectionTime
     * @param site
     */
    protected void setInfectionTime(final int infectionTime, final Site site) {
     ...
     * @param time
     */
    public void setScreenTime(final int time) {
        screenTime = time;
    }
     ...
     * @param screen
     */
    public void setScreenCycle(final int screen) {
        screenCycle = screen;
    }
    ...
    /**
     * May be overridden for specific agent subclasses with specific screening
     * behaviours
     * 
     * @param args (Object[]) for compatability with inherited versions which need
     *             input parameters
     * @return Probability of screening when symptomatic is false
     */
    public double getScreenProbability(final Object[] args) {
        return SCREEN_PROBABILITY;
    }

    /**
     * Invoked when Agent is symptomatic. Call each site.treat(). If all treatments
     * successful, call clearSymptomatic()
     * 
     * @return true if all sites successfully treated, false otherwise
     */
    public boolean treatSymptomatic() {
        final Site[] sites = getSites();
    ...
    /**
     * Invoked when Agent is asymptomatic. Call each site.treat(). If all treatments
     * successful, call clearSymptomatic()
     * 
     * @return true if all sites successfully treated, false otherwise
     */
    ...
    /**
     * Invoked when Agent is asymptomatic. Call site.treat(). If all treatments
     * successful, call clearSymptomatic()
     * 
     * @param site
     * @return true if all sites successfully treated, false otherwise
     */
    public boolean treat(final Site site) {
        final boolean successful = site.treat(); //
        if (successful) {
            infectedStatus = false;
            clearSymptomatic();
            screenTime = screenCycle;
    ...
    /**
     * Set symptomatic to false. Called only when all sites have been successfully
     * treated.
     */
    ...
    /**
     * Clear infection from all Sites and reset infectedStatus and symptomatic,
     * accordingly.
     */
     ...
     */
    public int getInfectedStatus(final Site site) {
        return site.getInfectedStatus();
    }

    /**
     * Setter of infectedStatus, and also ensures that symptomatic is false if
     * infectedStatus is false.
     * 
     * @param infected
     */
    public void setInfectedStatus(final boolean infected) {
        infectedStatus = infected;
        symptomatic = symptomatic && infectedStatus;
    }

    /**
     * Update symptomatic and infectedStatus according to the status of all Sites.
     */
    ...
    /**
     * Progress infection, invoking each site.progressSitesInfection and keeping
     * track of whether the Agent is still infected.
     * 
     ...
     */
    public boolean consent(final Object[] consentArgs) {
     ...
     * Currently according to whether in a Monogomous relationship and the number of
     * relationships already entered compared to concurrency. It is advisable for
     * subclasses of Agent to invoke this Method if they override with
     * super.consent() .
     * 
     ...
     */
    public boolean consent(final String relationshipClazzName, final Agent partner) {
        if (inMonogomous)
    ...
    /**
     * Randomly choose whether Agent is interested in a Relationship of Class
     * relationshipClazzName.
     * 
     * @param (String) relationshipClazzName
     * @return (boolean)
     */
    public boolean seekRelationship(final String relationshipClazzName) {
        if (inMonogomous)
            if (RAND.nextDouble() > infidelity)
                return false;
    ...
    /**
     * Whether to accept proposed Casual Relationship. DEPRECATED.
     * 
     ...
     */
    final public boolean joinGroupSexEvent(final Object[] args) {
        if (inMonogomous)
            if (RAND.nextDouble() > infidelity)
                return false;
    ...
    /**
     * Sets the availability according to the number of relationships and
     * participation in a Regular relationship
     * 
    ...
    /**
     * Adds relationship to ArrayList relationships and initiates entry into a
     * relationship
     * 
     * @param relationship
     * @return String report number of Relationships
     */
    public String enterRelationship(final Relationship relationship) {
        // String record = Reporter.ADD_REPORT_PROPERTY(Reporter.AGENTID0, agentId) ;
        final int partnerId = relationship.getPartnerId(agentId);
    ...
    public String augmentLowerAgentId() {
        ...
        final int lowerAgentInt = (int) Math.pow(2, (nbRelationships - 1));
        if ((lowerAgentId & lowerAgentInt) == lowerAgentInt)
            LOGGER.log(Level.SEVERE, "agentId:{0} already lowerAgentId of Relationship with {3} {1} {2}",
                    new Object[] { agentId, lowerAgentId, lowerAgentInt,
                            currentRelationships.get((nbRelationships - 1)).getPartnerId(agentId) });
        lowerAgentId = lowerAgentId ^ lowerAgentInt;
        return Integer.toString(lowerAgentId);
    }

    /**
     * For ending Relationships for which this Agent has the lower agentId. Agent
     * now knows that they are no longer the lowestAgentId Agent of relationship.
     * 
     * @param (Relationship) relationship
     * @return
     */
    public String diminishLowerAgentId(final Relationship relationship) {
        final int relationshipIndex = currentRelationships.indexOf(relationship);
        ...
        final int lowerAgentInt = (int) Math.pow(2, relationshipIndex);

        /*
         * if ((lowerAgentId & lowerAgentInt) != lowerAgentInt) LOGGER.log(Level.SEVERE,
         * "agentId:{0} lowerAgentId:{1} lowerAgentInt:{2} nbRelationships:{3}", new
         * Object[]{agentId,lowerAgentId,lowerAgentInt,nbRelationships});
         */

        // All bits up to and including relationshipIndex
        final int smallerIndexMask = (2 * lowerAgentInt) - 1;

        // Keep only bits less than relationshipIndex
        final int smallerIndex = lowerAgentId & (smallerIndexMask >> 1);
    ...
    /**
     * To avoid double application of Relationship.Methods() we act from the Agent
     * with the smallest agentId
     * 
     * @param (Relationship) relationship
     * @return String record of relationship.relationshipId
     */
    private String endRelationship(final Relationship relationship) {
        String record = "";
    ...
    /**
     * Override and call in subclass if details about possible partners are needed.
     * TODO: Currently not used but may be in future if Relationship subclass and
     * Agent partner are no longer enough data
     * 
     * @param relationshipClazzName
     * @param agent
     * @return String[] args of relationshipClazzName and other Properties relevant
     *         to deciding consent()
     */
    protected Object[] consentArgs(final String relationshipClazzName, final Agent agent) {
        final Object[] consentArgs = { relationshipClazzName, agent };
        return consentArgs;
    }
     ...
     * @param relationship
     */
    public void leaveRelationship(final Relationship relationship) {
        diminishLowerAgentId(relationship);
        // Leave specific relationship subclass
        try {
            final String leaveMethodName = "leave" + relationship.getRelationship();
            final Method leaveRelationshipMethod = Agent.class.getMethod(leaveMethodName, Relationship.class);
         ...
         * for (int partnerI : currentPartnerIds) { debug += Integer.toString(partnerI)
         * + " " ; debug += Integer.toString(currentPartnerIds.indexOf(partnerI)) +
         * " : "; } LOGGER.info(debug);
         */
        final int partnerId = relationship.getPartnerId(agentId);
        final int partnerIndex = currentPartnerIds.indexOf(partnerId);
    ...
    /**
     * Ends Relationship with the agent whose agentId is specified. TODO: Check that
     * it works correctly and fix if so.
     * 
     * @param agentNb
     */
    private void leavePartnerId(final int agentNb) {
        final int partnerIndex = currentPartnerIds.indexOf(agentNb);
        final Relationship doomedRelationship = currentRelationships.get(partnerIndex);
        leaveRelationship(doomedRelationship);
    }

    public void leaveCasual(final Relationship relationship) {
        casualNumber--;
    }

    public void leaveRegular(final Relationship relationship) {
        regularNumber--;
    }

    public void leaveMonogomous(final Relationship relationship) {
        inMonogomous = false;
    }
    ...
    protected double getRisk() {
        ...
        if (ageYears > 65)
            ...
            risk = RISK_15;
        final double noRisk = Math.pow((1 - risk / 1000), 1.0 / DAYS_PER_YEAR);
        return 1 - noRisk;
    }

    /**
     * Make agent die and clear their Relationships Default based on startAge,
     * uniform probability with cutoff at maxLife
     */
    private void death() {
    ...
    final private void clearRelationships() {
        ...
        final int startIndex = (nbRelationships - 1);
        String record = "";
        for (int relationshipIndex = startIndex; relationshipIndex >= 0; relationshipIndex--) {
            relationship = currentRelationships.get(relationshipIndex);
            final Agent agentLowerId = relationship.getLowerIdAgent();
